def openwin():
    ...
    def test():
        ...
        if not role:
            return tools.show_error(
                tr(
                    '"The reference audio path name and the text corresponding to the reference audio must be filled in the settings"'
                )
            )
    ...
    def save():
        ...
        if url in ("http://", "https://"):
            return tools.show_error(tr("Please fill in the correct CosyVoice api address"))
        role = winobj.role.toPlainText().strip()
        if not role:
            return tools.show_error(tr("Please upload reference audio in wav format"))
