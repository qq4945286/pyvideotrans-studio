...
def openwin():
    ...
    def get_file(type):
        if type == 1:
            ...
            fname, _ = QFileDialog.getOpenFileNames(
                winobj, "Select Video", params["last_opendir"], f"Video files({format_str})"
            )
            if len(fname) > 0:
            ...
            fname, _ = QFileDialog.getOpenFileName(
                winobj, "Select Image", params["last_opendir"], "files(*.png *.jpg *.jpeg *.gif)"
            )
            if fname:
    ...
    def start():
        ...
        task = CompThread(
            ...
            width=max(w, 1),
            height=max(h, 1),
            pos=int(winobj.compos.currentIndex()),
        )
