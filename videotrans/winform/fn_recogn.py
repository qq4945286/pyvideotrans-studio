def openwin():
    ...
    def show_xxl_select():
        ...
        if sys.platform != "win32":
            ...
            return False
        if not settings.get("Faster_Whisper_XXL") or not Path(settings.get("Faster_Whisper_XXL", "")).exists():
            from PySide6.QtWidgets import QFileDialog

            exe, _ = QFileDialog.getOpenFileName(winobj, tr("Select faster-whisper-xxl.exe"), "C:/", f"Files(*.exe)")
            if exe:
    ...
    def model_type_change():
        ...
        recogn_type = winobj.shibie_recogn_type.currentIndex()
        is_allow_lang = recognition.is_allow_lang(
            langcode=lang, recogn_type=recogn_type, model_name=winobj.shibie_model.currentText()
        )
    ...
    def recogn_type_change():
        ...
        if recogn_type not in [
            ...
            if recogn_type in [
                ...
                winobj.shibie_model.addItems(
                    ...
                    if recogn_type != recognition.OPENAI_WHISPER
                    else contants.Openai_Whisper_Models
                )
        ...
        lang = translator.get_code(show_text=winobj.shibie_language.currentText())
        is_allow_lang_res = recognition.is_allow_lang(
            langcode=lang, recogn_type=recogn_type, model_name=winobj.shibie_model.currentText()
        )
        if is_allow_lang_res is not True:
            winobj.loglabel.setText(is_allow_lang_res)
