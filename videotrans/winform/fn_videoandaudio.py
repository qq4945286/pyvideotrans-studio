...
def openwin():
    ...
    class CompThread(QThread):
        ...
        def run(self) -> None:
            ...
            if not vailfiles:
                self.post(type="error", text=tr("Video and audio of the same name do not exist and cannot be merged"))
                return
    ...
    def start():
        ...
        if not folder or not Path(folder).exists() or not Path(folder).is_dir():
            tools.show_error(tr("You must select the folder where the video and audio with the same name exists."))
            return

        winobj.startbtn.setText(tr("under implementation in progress..."))
        winobj.loglabel.setText("")
        ...
        task = CompThread(
            parent=winobj,
            folder=folder,
            remain=winobj.remain.isChecked(),
