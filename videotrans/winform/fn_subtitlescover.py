def openwin():
                ...
                for i, v in enumerate(self.subtitlefiles):
                    ...
                    if self.target_format == "txt":
                        if raw_path.name.lower().endswith(".srt"):
                            ...
                            tools.runffmpeg(["-y", "-i", os.path.normpath(v), tmp_srt])
                        ...
                        continue
                    tools.runffmpeg(
                        ["-y", "-i", os.path.normpath(v), RESULT_DIR + f"/{Path(v).stem}.{self.target_format}"]
                    )
    ...
    def get_file():
        fnames, _ = QFileDialog.getOpenFileNames(
            winobj, tr("selectmp4"), params.get("last_opendir", ""), "Subtitles files(*.srt *.vtt *.ass)"
        )
        if len(fnames) < 1:
            return
        winobj.subtitlefiles = []
    ...
    def start():
        ...
        task = CompThread(parent=winobj, subtitlefiles=winobj.subtitlefiles, target_format=target_format)
