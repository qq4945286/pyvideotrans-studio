...
def openwin():
    ...
    class CompThread(QThread):
        ...
        def __init__(self, *, parent=None, folder=None, is_soft=False, language=None, maxlen=30, remain_hr=False):
        ...
        def run(self):
            ...
            if not vailfiles:
                self.post(type="error", text=tr("Video and srt of the same name do not exist and cannot be merged"))
                return
            percent = 0
            self.post(
                type="logs",
                text=tr(
                    "There are {} sets of videos with the same name and srt subtitles that need to be merged.", length
                ),
            )
            for name, info in vailfiles.items():
                try:
                    ...
                    cmd = ["-y", "-i", os.path.normpath(info["video"])]
    ...
    def get_file():
        dirname = QFileDialog.getExistingDirectory(winobj, tr("selectsavedir"), params.get("last_opendir", ""))
    ...
    def start():
        ...
        winobj.startbtn.setText(tr("In Progress..."))
