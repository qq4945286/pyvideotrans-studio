...
def openwin():
                ...
                if self.audio:
                    ...
                    if audio_time < self.video_time:
                        audio_data = AudioSegment.from_file(
                            self.audio, format="mp4" if ext == "m4a" else ext
                        ) + AudioSegment.silent(duration=self.video_time - audio_time)
                        ...
                        tools.precise_speed_up_audio(
                            file_path=self.audio, out=tmp_audio, target_duration_ms=self.video_time
                        )
                        self.audio = tmp_audio
                    ...
                    if self.saveraw and self.video_info["streams_audio"]:
                        ...
                        # 先取出来视频中的音频为 wav
                        tools.runffmpeg(["-y", "-i", os.path.normpath(self.video), "-vn", tmp_mp4a])
                    ...
                    if self.audio_process == 2 and audio_time > self.video_time:
                        ...
                        cmd += ["-vf", f"tpad=stop_mode=clone:stop_duration={sec}"]
                    cmd += ["-an", "-c:v", f'libx{settings.get("video_codec", 264)}', novoice_mp4]
    ...
    def get_file(type="video"):
        ...
        if type == "video":
            ...
            fname, _ = QFileDialog.getOpenFileName(
                winobj, "Select Video", params.get("last_opendir", ""), f"Video files({format_str})"
            )
        elif type == "wav":
            format_str = " ".join(["*." + f for f in contants.AUDIO_EXITS])
            fname, _ = QFileDialog.getOpenFileName(
                winobj, "Select Audio", params.get("last_opendir", ""), f"Audio files({format_str})"
            )
        elif type == "srt":
            fname, _ = QFileDialog.getOpenFileName(
                winobj, "Select SRT", params.get("last_opendir", ""), "Srt files(*.srt)"
            )
    ...
    def start():
        ...
        if not audio and not srt:
            tools.show_error(tr("Choose at least one for audio and video"))
            return

        winobj.ysphb_startbtn.setText(tr("In Progress..."))
