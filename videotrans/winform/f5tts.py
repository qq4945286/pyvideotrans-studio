def openwin(init=False):
    ...
    def test(tts_type=tts.F5_TTS):
        ...
        wk = ListenVoice(
            ...
            queue_tts=[
                {
                    "text": "你好啊我的朋友,希望你今天开心！",
                    "role": role_test,
                    "filename": TEMP_DIR + f"/{time.time()}-{tts_type}.wav",
                    "tts_type": tts_type,
                }
            ],
            language="zh",
    ...
    def getrole():
        ...
        for it in tmp.split("\n"):
            ...
            if len(s) != 2:
                tools.show_error(
                    tr(
                        "Each line must be split into two parts with #, in the format of audio name.wav#audio text content"
                    )
                )
                return
