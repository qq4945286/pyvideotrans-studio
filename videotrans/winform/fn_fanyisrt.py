...
def openwin():
    ...
    def fanyi_import_fun():
        fnames, _ = QFileDialog.getOpenFileNames(
            winobj, tr("tuodongfanyi"), params["last_opendir"], "Subtitles files(*.srt)"
        )
        if len(fnames) < 1:
            return
        namestr = []
        ...
        if fnames:
            ...
            winobj.fanyi_sourcetext.setPlainText(f'{tr("yidaorujigewenjian")}{len(fnames)}\n{",".join(namestr)}')
    ...
    def fanyi_start_fun():
        ...
        source_code, target_code = translator.get_source_target_code(
            show_source=winobj.fanyi_source.currentText(), show_target=target_language, translate_type=translate_type
        )
    ...
    def target_lang_change(t):
        ...
        # 判断翻译渠道是否支持翻译到该目标语言
        if (
            translator.is_allow_translate(translate_type=winobj.fanyi_translate_type.currentIndex(), show_target=t)
            is not True
        ):
            return
    ...
    def translate_type_change(idx):
        translator.is_allow_translate(
            translate_type=winobj.fanyi_translate_type.currentIndex(), show_target=winobj.fanyi_target.currentText()
        )
        show_model_list()
