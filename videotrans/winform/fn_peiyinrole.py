def openwin():
    ...
    def hecheng_start_fun():
        ...
        if not winobj.srt_path:
            return tools.show_error(tr("Please import an SRT subtitle file first."))
    ...
    def hecheng_import_fun():
        fname, _ = QFileDialog.getOpenFileName(winobj, "SRT", params.get("last_opendir", "."), "SRT files (*.srt)")
        if not fname:
            return
