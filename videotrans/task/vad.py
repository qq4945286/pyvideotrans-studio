...
def get_speech_timestamp_silero(
    input_wav, threshold=None, min_speech_duration_ms=None, max_speech_duration_ms=None, min_silent_duration_ms=None
):
    # 防止填写错误
    ...
    speech_chunks = get_speech_timestamps(
        decode_audio(input_wav, sampling_rate=sampling_rate), vad_options=VadOptions(**vad_p)
    )
...
def get_speech_timestamp(
    ...
    for s, e in initial_segments:
        ...
        if duration > (max_frames_limit + 125):
            ...
            sub_segs = _detect_raw_segments(
                sub_data, adjusted_threshold, tighter_min_sil_frames, max_speech_frames=max_frames_limit
            )
    ...
    for s, e in refined_segments:
        ...
        while (end_ms - curr_s) > max_speech_duration_ms:
            ...
            # 寻找最后一个静音区域
            block_segments = _detect_raw_segments(
                block_data, adjusted_threshold, min_sil_frames / 2, max_speech_frames=None
            )
