...
class DubbingSrt(BaseTask):
    ...
    def _tts(self) -> None:
        ...
        if _enter_edgetts_single:
            ...
            tmp_name = (
                self.cfg.target_wav
                if self.cfg.target_wav.endswith(".mp3")
                else f"{self.cfg.cache_folder}/{self.cfg.noextname}-edgetts-txt-{time.time()}.mp3"
            )
            if self.cfg.target_sub.endswith(".txt"):
    ...
    def align(self) -> None:
        ...
        try:
            ...
            if target_path.is_file() and target_path.stat().st_size > 0:
                self.cfg.target_wav = (
                    self.cfg.target_wav[:-4]
                    + f'-{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}{target_path.suffix}'
                )
            ...
            rate_inst = TtsSpeedRate(
                ...
                uuid=self.uuid,
                shoud_audiorate=(
                    self.cfg.voice_autorate if not self.cfg.target_sub.endswith(".txt") else False
                ),  # txt 配音禁止自动加速，需要移除字幕静音，即直接相连即可
                raw_total_time=self.queue_tts[-1]["end_time"],
                target_audio=self.cfg.target_wav,
                cache_folder=self.cfg.cache_folder,
                remove_silent_mid=(
                    self.cfg.remove_silent_mid if not self.cfg.target_sub.endswith(".txt") else True
