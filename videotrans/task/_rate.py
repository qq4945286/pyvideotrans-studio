...
def _build_video_cut_cmd(
    *,
    ...
    """
    根据编码器构建 ffmpeg 命令。支持 VAAPI/NVENC/AMF/QSV/videotoolbox 硬件加速。
    """
    hw_init = []
    hw_post_filter = None
    enc_params = ["-crf", crf]
    ...
    if codec.endswith("_vaapi"):
        ...
        hw_post_filter = "format=nv12,hwupload=derive_device=vaapi"
        # VAAPI QP 比 libx264 CRF 约高 6 档才有相近压缩率/画质
        enc_params = ["-qp", str(max(1, min(51, int(crf) + 6)))]
        pix_fmt = []  # VAAPI 不需要 pix_fmt
    elif codec.endswith("_nvenc"):
        # NVENC CQ 比 CRF 约高 3 档
        enc_params = ["-cq", str(max(1, min(51, int(crf) + 3)))]
        preset_map = {"ultrafast": "p2", "veryfast": "p2", "fast": "p3", "medium": "p4", "slow": "p6"}
        actual_preset = preset_map.get(preset, "p4")
    elif codec.endswith("_amf"):
        # AMF QP_p 比 CRF 高 8 档
        enc_params = ["-qp_p", str(max(1, min(51, int(crf) + 8)))]
    elif codec.endswith("_qsv"):
        # QSV global_quality 接近 CRF，微调 +2
        enc_params = ["-global_quality", str(max(1, min(51, int(crf) + 2)))]
        hw_post_filter = "hwupload=extra_hw_frames=64"
        pix_fmt = []  # QSV 通过 hwupload 处理
    elif codec.endswith("_videotoolbox"):
        enc_params = ["-q:v", crf]
        pix_fmt = []

    # filter chain: setpts (软件) → hwupload (硬件上传)
    all_filter_parts = list(filter_parts)
    if hw_post_filter:
        all_filter_parts.append(hw_post_filter)

    cmd = ["-y"]
    cmd.extend(hw_init)
    cmd.extend(["-ss", ss_time, "-t", f"{source_duration_s:.6f}"])
    cmd.extend(["-i", input_video_path])
    # -g 1 确保所有帧为关键帧，保证拼接兼容性
    cmd.extend(["-an", "-c:v", codec, "-preset", actual_preset, "-g", "1"])
    cmd.extend(enc_params)
    cmd.extend(pix_fmt)
    cmd.extend(["-vf", ",".join(all_filter_parts)])
...
def _cut_video_get_duration(i, task, novoice_mp4_original, preset, crf):
    ...
    自动检测 GPU 硬件编码器（VAAPI/NVENC/AMF/QSV），回退到 libx264。
    """
    task["actual_duration"] = 0

    # 自动检测最佳编码器
    vcodec = tools.get_video_codec()
    is_hw = vcodec not in ("libx264", "libx265")
    ...
    # 构建 filter_parts（仅 filter 逻辑，不含 hwupload）
    filter_parts = []
    if abs(pts_factor - 1.0) >= 0.001:
        filter_parts.append(f"setpts={pts_factor+0.005}*PTS")
    else:
        filter_parts.append("setpts=PTS")

    flag = f'[Video-Cut] 片段{i} [原:{task["start"]}-{task["end"]}ms] [目标:{target_duration_ms}ms] [PTS:{pts_factor:.4f}] [编码:{vcodec}]'
    logger.debug(f"{flag} 准备开始处理...")

    work_dir = Path(task["filename"]).parent.as_posix()
    file_path = Path(task["filename"])
    output_basename = os.path.basename(task["filename"])

    # 尝试策略：HW 编码 → libx264 软编码 → libx264 无变速兜底
    strategies = []
    if is_hw:
        strategies.append((vcodec, True, False))  # HW + PTS
    strategies.append(("libx264", True, True))  # libx264 + PTS (force_cpu)
    strategies.append(("libx264", False, True))  # libx264 无变速 (force_cpu)

    success = False
    for enc_codec, use_pts, force_cpu in strategies:
        if success:
            break
        try:
            # 清理之前失败的文件
        ...
        try:
            cmd, _ = _build_video_cut_cmd(
                ...
                preset=preset,
                crf=crf,
                filter_parts=filter_parts if use_pts else ["setpts=PTS"],
                target_duration_s=target_duration_s,
                output_filename=output_basename,
                use_pts=use_pts,
            )
            tools.runffmpeg(cmd, force_cpu=force_cpu, cmd_dir=work_dir)
            if file_path.exists() and file_path.stat().st_size >= 1024:
                success = True
            else:
                logger.warning(f"{flag} {enc_codec} {'变速' if use_pts else '无变速'} 输出无效，尝试下一策略")
        except Exception as e:
            logger.warning(f"{flag} {enc_codec} {'变速' if use_pts else '无变速'} 执行异常: {e}")
    ...
    if success:
        ...
        logger.debug(f"{flag} 完成。实际生成时长: {real_time}ms 编码器: {vcodec if is_hw and success else 'libx264'}")
    else:
        task["actual_duration"] = 0
        logger.error(f"{flag} 所有策略均失败。")
        try:
            if file_path.exists():
                file_path.unlink(missing_ok=True)
        except:
...
def _change_speed_rubberband(input_path, target_duration):
    ...
    try:
        ...
        if target_duration > current_duration:
            # 允许微小的误差，或者由后续静音填充处理
            logger.debug(
                f"[Audio-RB] 目标时长({target_duration}) > 当前时长({current_duration})，跳过变速，交由静音填充。"
            )
            return
...
class SpeedRate:
    ...
    def __init__(
        ...
        *,
        ...
        self.cache_folder = (
            cache_folder if cache_folder else Path(f"{TEMP_DIR}/{str(uuid if uuid else time.time())}").as_posix()
        )
        ...
        self.max_audio_speed_rate = float(settings.get("max_audio_speed_rate", 2.0))
        ...
        logger.debug(
            f"[SpeedRate] Init. AudioRate={self.shoud_audiorate}, VideoRate={self.shoud_videorate}, Rubberband={bool(self.audio_speed_rubberband)}"
        )
    ...
    def run(self):
        ...
        if self.shoud_videorate and self.video_for_clips:
            ...
            # 不变视频，时长为原槽位时长（若音频加速阶段已延伸则不覆盖）
            for it in self.queue_tts:
                if "final_duration" not in it:
    ...
    def _calculate_adjustments(self):
        ...
        for i, it in enumerate(self.queue_tts):
            ...
            if self.shoud_audiorate and not self.shoud_videorate:
                ...
                if dubb_dur > source_dur:
                    ...
                    if ratio > self.max_audio_speed_rate:
                        ...
                        it["final_duration"] = audio_target
                    else:
                        audio_target = source_dur
            ...
            if self.shoud_audiorate and audio_target < dubb_dur:
                self.audio_data.append({"filename": it["filename"], "dubb_time": dubb_dur, "target_time": audio_target})
            ...
            # 记录决策日志
            logger.debug(
                f"[Calc] Mode={mode_log} Line={it['line']} | Source={source_dur} Dubb={dubb_dur} -> TargetV={video_target} TargetA={audio_target}"
            )
        ...
        for d in self.audio_data:
            all_task.append(
                GlobalProcessManager.submit_task_cpu(
                    _change_speed_rubberband, input_path=d["filename"], target_duration=d["target_time"]
                )
            )
    ...
    def _concat_audio_aligned(self):
        ...
        for i, it in enumerate(self.queue_tts):
            ...
            if slot_duration <= 0:
                logger.warning(
                    f"[Audio-Sync] 字幕[{it['line']}] 视频槽时长为0，回退使用原始时长: {it['source_duration']}ms"
                )
            ...
            logger.debug(
                f"[Audio-Sync] Line={it['line']} | {log_flag} | [{current_slot_audio_len=} {slot_duration=}] | Timeline: {current_timeline} -> {current_timeline+slot_duration}"
            )
    ...
    def _create_silen_file(self, name, duration_ms):
        ...
        AudioSegment.silent(duration=duration_ms, frame_rate=self.AUDIO_SAMPLE_RATE).set_channels(
            self.AUDIO_CHANNELS
...
class TtsSpeedRate(SpeedRate):
    ...
    def _calculate_adjustments(self):
        ...
        for i, it in enumerate(self.queue_tts):
            ...
            logger.debug(
                f"[Calc] Mode={mode_log} Line={it['line']} | Source={source_dur} Dubb={dubb_dur} -> TargetA={audio_target}"
            )
