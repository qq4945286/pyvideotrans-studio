    ...
    def run(self):
        try:
            ...
            if p.suffix.lower() != ".wav":
                ...
                cmd = ["-y", "-i", self.file, "-vn", "-ac", "1", "-ar", "44100", newfile]
                tools.runffmpeg(cmd)
                self.file = newfile
            tools.set_process(uuid=self.uuid)
            kw = {
                "input_file": self.file,
                "vocal_file": self.vocal,
                "instr_file": f"{self.out}/instrument-{p.stem}.wav",
                "uvr_models": uvr_models,
            }
            future = GlobalProcessManager.submit_task_cpu(vocal_bgm, **kw)
