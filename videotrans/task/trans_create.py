...
from videotrans.recognition import (
    run as run_recogn,
    Faster_Whisper_XXL,
    Whisper_CPP,
    is_allow_lang as recogn_allow_lang,
    FASTER_WHISPER,
)
...
class TransCreate(BaseTask):
    ...
    def __post_init__(self):
        ...
        if self.cfg.clear_cache:
            # 仅清空缓存子目录，保留字幕等输出文件
            for _d in (f"{self.cfg.target_dir}/cache", f"{self.cfg.target_dir}/cache-work"):
                if Path(_d).is_dir():
                    shutil.rmtree(_d, ignore_errors=True)
        self._signal(text=tr("kaishichuli"))
        # 配音溢出列表，dubbing() 后填充
        self._dubbing_overflow: list = []
        ...
        # 临时工作目录，放在用户输出目录下，合成后不自动删除
        if not self.cfg.cache_folder:
            self.cfg.cache_folder = f"{self.cfg.target_dir}/cache-work"
        # 输出文件夹内的配音缓存目录，存放逐条配音文件
        self.cfg.dubb_cache = f"{self.cfg.target_dir}/cache"
        # 检测字幕原始语言
    ...
    def prepare(self) -> None:
        ...
        # 断点续跑：已有检查点记录则保护已完成步骤的产物
        _ckpt = os.path.join(self.cfg.target_dir, ".pipeline_state.json")
        ...
        if audio_stream_len < 1 and not tools.vail_file(self.cfg.source_sub):
            ...
            raise RuntimeError(
                tr(
                    "There is no valid audio in the file {} and it cannot be processed. Please play it manually to confirm that there is sound.",
                    self.cfg.name,
                )
            )
        ...
        if self.cfg.subtitles.strip():
            with open(self.cfg.source_sub, "w", encoding="utf-8", errors="ignore") as f:
                txt = re.sub(
                    r":\d+\.\d+", lambda m: m.group().replace(".", ","), self.cfg.subtitles.strip(), flags=re.I | re.S
                )
                f.write(txt)
        ...
        if (
            audio_stream_len > 0
            and self.cfg.is_separate
            and (not tools.vail_file(self.cfg.vocal) or not tools.vail_file(self.cfg.instrument))
        ):
        ...
        if audio_stream_len > 0 and not tools.vail_file(self.cfg.source_wav) and tools.vail_file(self.cfg.vocal):
            ...
            cmd = ["-y", "-i", self.cfg.vocal, "-ac", "1", "-ar", "16000", "-c:a", "pcm_s16le", self.cfg.source_wav]
            try:
        ...
        # 立即将提取的音频副本保存到输出目录
        if tools.vail_file(self.cfg.source_wav) and self.cfg.source_wav_output:
            try:
                Path(self.cfg.source_wav_output).parent.mkdir(parents=True, exist_ok=True)
                tools.runffmpeg(
                    [
                        "-y",
                        "-i",
                ...
                logger.error(f"复制原始音频到输出目录失败:{e}")
    ...
    def recogn(self) -> None:
        ...
        if not self.cfg.is_separate and self.cfg.remove_noise:
            ...
            if not Path(_cache_noise_wav).exists():
                ...
                kw = {"input_file": self.cfg.source_wav, "output_file": _remove_noise_wav, "is_cuda": self.cfg.is_cuda}
                try:
        ...
        if self.cfg.recogn_type == Faster_Whisper_XXL:
            ...
            cmd = [xxl_path, self.cfg.source_wav, "-pp", "-f", "srt"]
            ...
            cmd = [cpp_path, "-f", self.cfg.source_wav, "-osrt", "-np"]
            ...
            if not Path(f"{cpp_folder}/models/{self.cfg.model_name}").is_file():
                raise RuntimeError(
                    tr(
                        "The model does not exist. Please download the model to the {} directory first.",
                        f"{cpp_folder}/models",
                    )
                )
        ...
        if self.cfg.fix_punc and self.cfg.detect_language[:2] in ["zh", "en"]:
            tools.check_and_down_ms(
                model_id="iic/punc_ct-transformer_cn-en-common-vocab471067-large", callback=self._process_callback
            )
            ...
            try:
                _rs = self._new_process(
                    callback=fix_punc, title=tr("Restoring punct"), is_cuda=self.cfg.is_cuda, kwargs=kw
                )
                if _rs:
                    for it in self.source_srt_list:
                        ...
                        if self.cfg.detect_language[:2] == "en":
                            it["text"] = (
                                it["text"].replace("，", ",").replace("。", ". ").replace("？", "?").replace("！", "!")
                            )
    ...
    def recogn2pass(self) -> None:
        ...
        if self.cfg.subtitle_type > 2 and (self.cfg.source_language_code != self.cfg.target_language_code):
            logger.debug(
                f"跳过二次识别, 因设置了嵌入双字幕，二次识别后双字幕时间戳将无法保持一致，因此跳过：{self.cfg.subtitle_type=}"
            )
            return
        ...
        try:
            ...
            if (
                recogn_allow_lang(
                    langcode=self.cfg.target_language_code, recogn_type=recogn_type, model_name=model_name
                )
                is not True
            ):
            ...
            if recogn_type == Faster_Whisper_XXL:
                xxl_path = settings.get("Faster_Whisper_XXL", "Faster_Whisper_XXL.exe")
                cmd = [xxl_path, shibie_audio, "-pp", "-f", "srt"]
                ...
                cpp_path = settings.get("Whisper_cpp", "whisper-cli")
                cmd = [cpp_path, "-f", shibie_audio, "-osrt", "-np"]
    ...
    def diariz(self):
        ...
        if (
            self._exit()
            or not self.cfg.enable_diariz
            or self.max_speakers == 1
            or Path(self.cfg.cache_folder + "/speaker.json").exists()
        ):
            return
        ...
        if speaker_type in ["pyannote", "reverb"] and not hf_token:
            logger.error(
                f"当前选择 pyannote 说话人分离模型，但未设置 huggingface.co 的token: {self.cfg.detect_language}"
            )
            return
        ...
        try:
            ...
            if speaker_type == "built":
                ...
                tools.check_and_down_ms(
                    model_id="iic/speech_campplus_speaker-diarization_common", callback=self._process_callback
                )
            ...
            if speaker_type in ["pyannote", "reverb"]:
                ...
                snapshot_download(
                    repo_id=(
                        ...
                        else "Revai/reverb-diarization-v1"
                    ),
                    token=hf_token,
                )

            spk_list = self._new_process(
                callback=_run_speakers, title=title, is_cuda=self.cfg.is_cuda and speaker_type != "built", kwargs=kw
            )
    ...
    def trans(self) -> None:
        ...
        try:
            ...
            # 仅提取，并且双语输出
            if (
                self.cfg.app_mode == "tiqu"
                and self.cfg.output_srt > 0
                and self.cfg.source_language_code != self.cfg.target_language_code
            ):
    ...
    def dubbing(self) -> None:
        ...
        try:
            self._tts()
            # TTS 完成后检查配音时长，对超出字幕时段的进行缓解处理
            self._dubbing_overflow = self._mitigate_dubbing_overflow()
        except Exception as e:
            self.hasend = True
            raise
    ...
    def align(self) -> None:
        ...
        if (
            ...
            volume = str(self.cfg.volume).replace("%", "").strip()
            try:
    ...
    def task_done(self) -> None:
        ...
        if self.cfg.app_mode == "tiqu":
            try:
                Path(f"{self.cfg.target_dir}/{self.cfg.source_language_code}.srt").unlink(missing_ok=True)
                Path(f"{self.cfg.target_dir}/{self.cfg.target_language_code}.srt").unlink(missing_ok=True)
            except:
                pass  # 忽略删除失败
        else:
    ...
    def _split_novoice_byraw(self):
        ...
        try:
            hw_decode_args, vf_string, vcodec, enc_args = self._get_hard_cfg(codec="264")
            cmd = [
                "-y",
                "-fflags",
                "+genpts",
            ...
            cmd += hw_decode_args
            cmd += ["-i", self.cfg.name, "-an", "-c:v", vcodec]
            if vf_string:
                cmd += ["-vf", vf_string]
            if enc_args:
                cmd += enc_args
            ...
            return tools.runffmpeg(
                ["-y", "-fflags", "+genpts", "-i", self.cfg.name, "-an", "-c:v", "libx264", _name],
                noextname=self.uuid,
                cmd_dir=self.cfg.cache_folder,
                force_cpu=True,
            )
    ...
    def _split_audio_byraw(self, is_separate=False):
        if not isinstance(self.cfg.source_wav, str) or not self.cfg.source_wav:
            logger.error(f"source_wav 无效: {self.cfg.source_wav!r}, 重新设置")
            self.cfg.source_wav = f"{self.cfg.cache_folder}/{self.cfg.source_language_code}.wav"
        cmd = ["-y", "-i", self.cfg.name, "-vn", "-ac", "1", "-ar", "16000", "-c:a", "pcm_s16le", self.cfg.source_wav]
        ...
        tools.runffmpeg(["-y", "-i", self.cfg.name, "-vn", "-ac", "2", "-ar", "44100", "-c:a", "pcm_s16le", tmpfile])
        ...
        kw = {
            "input_file": tmpfile,
            "vocal_file": self.cfg.vocal,
            "instr_file": self.cfg.instrument,
    ...
    def _tts(self, daz_json=None) -> None:
        ...
        # 确保配音缓存目录存在
        Path(self.cfg.dubb_cache).mkdir(parents=True, exist_ok=True)
        try:
        ...
        for i, it in enumerate(subs):
            ...
            tmp_dict = {
                ...
                "start_time_source": (
                    source_subs[i]["start_time"] if source_subs and i < len(source_subs) else it["start_time"]
                ),
                "end_time_source": (
                    source_subs[i]["end_time"] if source_subs and i < len(source_subs) else it["end_time"]
                ),
                "role": voice,
                "rate": rate,
        ...
        # 配音前预估：生成配音时长预估.txt 供用户参考
        self._estimate_tts_duration()
    ...
    def _re_tts(self, indices: list[int]) -> None:
        """仅对指定索引的字幕条目重新配音（增量重配音）"""
        if not indices:
            return
        re_queue = [self.queue_tts[i] for i in indices if i < len(self.queue_tts)]
        if not re_queue:
            return
        logger.info(f"[ReTTS] 重新配音 {len(re_queue)} 条字幕: indices={indices}")
        self._signal(text=f"重新配音 {len(re_queue)} 条字幕...")
        run_tts(
            queue_tts=re_queue,
            language=self.cfg.target_language_code,
            uuid=self.uuid,
    ...
    # 配音后时长检查与缓解处理
    def _estimate_tts_duration(self):
        """
        翻译完成后、TTS 之前预估每条字幕的配音时长。
        中文按 ~250ms/字，英文按 ~300ms/词 粗估，保存预估报告到输出目录。
        """
        report_path = f"{self.cfg.target_dir}/配音时长预估.txt"
        lang = self.cfg.target_language_code[:2] if self.cfg.target_language_code else "zh"
        lines = [
            "══════════════════════════════════════════",
            "  配音时长预估（仅供参考，实际以配音结果为准）",
            "══════════════════════════════════════════",
            "",
        ]
        has_warning = False
        for it in self.queue_tts:
            ...
            slot_dur = it["end_time"] - it["start_time"]
            if not text or slot_dur <= 0:
                continue
            # 粗估：中文按每字 250ms，其他语言按空格分词每词 300ms
            if lang == "zh":
                est_dur = len(text.replace(" ", "")) * 250
            else:
                est_dur = len(text.split()) * 300
            ratio = est_dur / slot_dur if slot_dur > 0 else 0
            flag = " ⚠️ 可能超时" if ratio > 1.0 else ""
            if ratio > 1.0:
                has_warning = True
            lines.append(f"字幕 #{it['line']}: 预估 {est_dur}ms / 时段 {slot_dur}ms (ratio={ratio:.2f}){flag}")
            lines.append(f"  文本: {text[:60]}{'...' if len(text) > 60 else ''}")
            lines.append("")
        if has_warning:
            lines.insert(3, "⚠️ 以下字幕预估配音时长可能超出原时段，建议精简翻译文本：")
        try:
            Path(report_path).write_text("\n".join(lines), encoding="utf-8")
            logger.info(f"[Estimate] 预估报告已保存至: {report_path}")
        except Exception as e:
            logger.error(f"[Estimate] 保存报告失败: {e}")

    def _mitigate_dubbing_overflow(self) -> list:
        """
        TTS 完成后检查每条配音时长：
        1. 若配音时长超出字幕时段但下条字幕间隔足够 → 延长本条结束时间
        2. 若超出 ≥15% → 加入 overflow 列表（阻断后续对齐，提示用户优化字幕）
        返回 overflow 列表，元素为 dict {line, text, dubb_time, slot_time, ratio}
        """
        overflow = []
        for i, it in enumerate(self.queue_tts):
            ...
            if not filename or not Path(filename).exists():
                continue
            try:
                dubb_dur = len(AudioSegment.from_file(filename))
            except Exception:
                continue
            slot_dur = it["end_time"] - it["start_time"]
            if slot_dur <= 0:
                continue
            ratio = dubb_dur / slot_dur

            if ratio <= 1.15:
                # 在可接受范围内，后续对齐用 rubberband 微调即可
                continue

            # 策略1: 延长本条字幕结束时间，借用与下条字幕的间隔
            if i < len(self.queue_tts) - 1:
                next_start = self.queue_tts[i + 1]["start_time"]
                # 至少保留 100ms 间隔
                max_extend = max(next_start - 100, it["end_time"])
                if max_extend > it["end_time"]:
                    # 最多借用到 max_extend，目标是将 ratio 降到 1.15 以下
                    target_slot = min(max_extend - it["start_time"], int(dubb_dur / 1.15))
                    new_end = it["start_time"] + target_slot
                    if new_end > it["end_time"]:
                        it["end_time"] = new_end
                        slot_dur = it["end_time"] - it["start_time"]
                        ratio = dubb_dur / slot_dur
                        it["endraw"] = tools.ms_to_time_string(ms=it["end_time"])
                        logger.debug(
                            f"[Overflow] 字幕[{it['line']}] 结束时间延长至 {it['end_time']}ms，新 ratio={ratio:.2f}"
                        )

            # 策略2: 仍超出 15% 阈值，记录到 overflow 列表
            if ratio > 1.15:
                overflow.append(
                    {
                        "line": it["line"],
                ...
                logger.warning(
                    f"[Overflow] 字幕[{it['line']}] 配音时长 {dubb_dur}ms 超出字幕时段 {slot_dur}ms (ratio={ratio:.2f})，需优化"
                )

        # 保存分析报告到输出目录
        if overflow:
            self._save_overflow_report(overflow)
        return overflow

    def _save_overflow_report(self, overflow: list):
        """保存配音溢出分析报告到输出目录"""
        report_path = f"{self.cfg.target_dir}/配音优化建议.txt"
        lines = [
            "══════════════════════════════════════════",
            "  配音优化建议 — 以下字幕配音时长超出原时段 ≥15%",
            "  请精简翻译文本后重新配音，避免音频强制变速失真",
            "══════════════════════════════════════════",
            "",
        ]
        for item in overflow:
            dubb_s = item["dubb_time"] / 1000.0
            slot_s = item["slot_time"] / 1000.0
            lines.append(f"字幕 #{item['line']}:")
            lines.append(f"  翻译文本: {item['text']}")
            lines.append(f"  配音时长: {dubb_s:.1f}s  |  字幕时段: {slot_s:.1f}s  |  超出: {item['ratio']:.0%}")
            lines.append("")
        try:
            Path(report_path).write_text("\n".join(lines), encoding="utf-8")
            logger.info(f"[Overflow] 分析报告已保存至: {report_path}")
        except Exception as e:
            logger.error(f"[Overflow] 保存报告失败: {e}")
    ...
    def _create_ref_from_vocal(self):
        ...
        def _cutaudio_from_vocal(it):
            try:
                ...
                tools.cut_from_audio(audio_file=vocal, ss=it["startraw"], to=it["endraw"], out_file=it["ref_wav"])
            except Exception as e:
    ...
    def _back_music(self) -> None:
        ...
        try:
            ...
            if settings.get("loop_backaudio") and beishu > 1 and vtime - 1000 > atime:
                ...
                tools.concat_multi_audio(concat_txt=concat_txt, out=self.cfg.cache_folder + "/bgm_file_extend.wav")
            ...
            tools.runffmpeg(
                [
                    ...
                    "-filter:a",
                    f"volume={settings.get('backaudio_volume', 0.8)}",
                    "-c:a",
                    "pcm_s16le",
            ...
            if atime + 1000 < vtime:
                if int(settings.get("loop_backaudio")) == 1:
                    ...
                    tools.concat_multi_audio(
                        concat_txt=concat_txt, out=self.cfg.cache_folder + "/instrument-concat.wav"
                    )
                else:
                    # 延长背景音
                    tools.change_speed_rubberband(
                        instrument_file, self.cfg.cache_folder + f"/instrument-concat.wav", vtime
                    )
    ...
    def _backandvocal(self, backwav, peiyinm4a):
        ...
        tools.runffmpeg(
            [
                "-y",
                "-i",
                os.path.basename(peiyinm4a),
                "-i",
                os.path.basename(tmpm4a),
                "-filter_complex",
                "[0:a][1:a]amix=inputs=2:duration=first:dropout_transition=2",
                "-ac",
                "2",
    ...
    def _process_subtitles(self) -> Union[tuple[str, str], None]:
        ...
        # 单行字符数
        maxlen = int(
            settings.get("cjk_len", 15)
            if self.cfg.target_language_code[:2] in ["zh", "ja", "jp", "ko", "yu"]
            else settings.get("other_len", 60)
        )
        ...
        if self.cfg.subtitle_type in [3, 4]:
            ...
            source_maxlen = int(
                settings.get("cjk_len", 15)
                if self.cfg.source_language_code[:2] in ["zh", "ja", "jp", "ko", "yu"]
                else settings.get("other_len", 60)
            )
            ...
            for i, it in enumerate(target_sub_list):
                ...
                if source_length > 0 and i < source_length:
                    _text_source = tools.simple_wrap(
                        source_sub_list[i]["text"], source_maxlen, self.cfg.source_language_code
                    )
                    _text = (
                        f"{_text_source}\n{_join_flag}{_text}"
                        if self.cfg.output_srt == 1
    ...
    def _video_extend(self, duration_ms=1000):
        ...
        cmd = [
            ...
            "-vf",
            f"tpad=stop_mode=clone:stop_duration={sec:.3f}",
            "-c:v",
            "libx264",
            "-crf",
            ...
            "-an",
            "final_video_with_freeze_lastend.mp4",
        ]
    ...
    def _join_video_audio_srt(self) -> None:
        ...
        if not self.shoud_dubbing:
            ...
            try:
                ...
                cmd = ["-y", "-i", self.cfg.name, "-vn", "-b:a", "128k", "-c:a", "aac", self.cfg.source_wav_output]
        ...
        try:
            ...
            cmd0 = ["-y", "-progress", protxt_basename]

            cmd1 = ["-i", novoice_mp4_basename, "-i", target_m4a_basename]
            ...
            if self.cfg.subtitle_type not in [1, 3]:
                ...
                if self.cfg.subtitle_type in [2, 4]:
                    cmd1.extend(["-c:s", "mov_text", "-metadata:s:s:0", f"language={subtitle_langcode}"])
    ...
    def _hebing_pro(self, protxt, video_time=0) -> None:
        while 1:
            if app_cfg.exit_soft or self.hasend or self.precent >= 100:
                return
    ...
    def _create_txt(self) -> None:
        try:
            ...
            with open(self.cfg.target_dir + f'/{tr("readme")}.txt', "w", encoding="utf-8", errors="ignore") as f:
                f.write(
    ...
    def _get_hard_cfg(self, subtitles_file=None, codec=None):
        ...
        # 全局参数，硬件解码相关
        global_args = []
        # 硬字幕嵌入参数，软字幕忽略；无字幕时不生成 subtitles 滤镜
        vf_string = ""
        if subtitles_file:
        ...
        if hw_type in ["nvenc"]:
            ...
            if settings.get("hw_decode"):
                ...
                if subtitles_file:
                    vf_string = (
                        f"[0:v]hwdownload,format=nv12,subtitles=filename='{subtitles_file}',hwupload_cuda[v_out]"
                    )
                else:
                    vf_string = ""
            else:
        ...
        if hw_type in ["qsv", "amf", "vaapi"]:
            if os_name == "Linux":
                ...
                if settings.get("hw_decode"):
                    ...
                    global_args = ["-init_hw_device", f"vaapi=vaapi:{device}"]
                    if subtitles_file:
                        vf_string = f"[0:v]subtitles=filename='{subtitles_file}',format=nv12,hwupload[v_out]"
                    else:
                        vf_string = "[0:v]format=nv12,hwupload[v_out]"
