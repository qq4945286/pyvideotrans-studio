...
class ColorPicker(QWidget):
    ...
    def update_swatch(self):
        self.color_swatch.setStyleSheet(
            f"background-color: rgba({self.color.red()},{self.color.green()},{self.color.blue()},{self.color.alpha()}); border: 1px solid black;"
        )
...
class PreviewWidget(QGraphicsView):
    ...
    def update_preview(self, style):
        ...
        text = (
            "你好啊，亲爱的朋友们！" if defaulelang == "zh" else "Hello, my dear friend. hope your every day beautiful"
        )
...
class ASSStyleDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        if DARK_STYLE_QSS:
            self.setStyleSheet(DARK_STYLE_QSS)
        ...
        positions = [(0, 0, 7), (0, 1, 8), (0, 2, 9), (1, 0, 4), (1, 1, 5), (1, 2, 6), (2, 0, 1), (2, 1, 2), (2, 2, 3)]
...
""")
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.bottom_primary_color_picker.color = ColorPicker.parse_color(
            ...
            # 第二个
            self.bottom_secondary_color_picker.color = ColorPicker.parse_color(
                style.get("Bottom_SecondaryColour", "&H00FFFFFF&")
            )
            self.bottom_secondary_color_picker.update_swatch()
            self.bottom_outline_color_picker.color = ColorPicker.parse_color(
