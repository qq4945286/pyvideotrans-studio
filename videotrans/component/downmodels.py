        ...
        try:
            ...
            if "urls" in self.task:
                ...
                for url in self.task["urls"]:
                    ...
                    urls_to_process.append({"url": url, "path": target_dir / fname, "extract": False})
            ...
            # 情况 B: 单个 URL (VITS/OpenAI 等)
                ...
                if is_archive:
                    ...
                    urls_to_process.append({"url": url, "path": None, "extract": True, "fname": fname})  # 临时处理
                else:
            ...
            for index, item in enumerate(urls_to_process):
                if self.isInterruptionRequested():
                    raise UserWarning(TRANS["msg_user_stopped"])
                ...
                with requests.get(url, stream=True, timeout=60) as response:
                    ...
                    if item["extract"]:
                        ...
                        if fname.endswith(".7z"):
                            if not HAS_7Z:
                                raise ImportError(TRANS["err_no_py7zr"])
                        else:
            ...
            if "hf-mirror.com" in urls_to_process[0]["url"] or "huggingface.co" in urls_to_process[0]["url"]:
                ...
                # zip 文件
                msg = TRANS["zip_xiazaishibai_shoudong"].format(
                    urls_to_process[0]["url"], self.task.get("zip_folder", ""), self.task["dir"]
                )
                QApplication.clipboard().setText(urls_to_process[0]["url"])
            else:
                # openai-whisper 模型，单个pt文件
                msg = TRANS["pt_xiazaishibai_shoudong"].format(
                    urls_to_process[0]["url"], self.task["name"], self.task["dir"]
                )
                QApplication.clipboard().setText(urls_to_process[0]["url"])
...
class TaskWidget(QFrame):
    ...
    def init_ui(self):
        ...
        self.pbar.setStyleSheet(
            "QProgressBar { height: 6px; border-radius: 3px; background: #666; } QProgressBar::chunk { background: #0078d7; border-radius: 3px; }"
        )
    ...
    def on_finished(self, success, msg):
        ...
        if success:
            ...
            QMessageBox.critical(
                self, TRANS["msg_error_title"], TRANS["msg_task_failed"].format(self.task["name"], msg)
            )
...
class DownmodelsWindow(QWidget):
        ...
        self.tasks_openai = [
            {
                ...
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}tiny.en.pt",
            },
            {
                "name": "tiny.pt",
                "url": "https://openaipublic.azureedge.net/main/whisper/models/65147644a518d12f04e32d6f3b26facc3f8dd46e5390956a9424a650c0ce22b9/tiny.pt",
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}tiny.pt",
            },
            {
                "name": "base.en.pt",
                ...
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}base.en.pt",
            },
            {
                "name": "base.pt",
                "url": "https://openaipublic.azureedge.net/main/whisper/models/ed3a0b6b1c0edf879ad9b11b1af5a0e6ab5db9205f891f668f8b0e6c6326e34e/base.pt",
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}base.pt",
            },
            {
                "name": "small.en.pt",
                ...
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}small.en.pt",
            },
            {
                "name": "small.pt",
                "url": "https://openaipublic.azureedge.net/main/whisper/models/9ecf779972d90ba49c06d968637d720dd632c55bbf19d441fb42bf17a411e794/small.pt",
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}small.pt",
            },
            {
                "name": "medium.en.pt",
                ...
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}medium.en.pt",
            },
            {
                "name": "medium.pt",
                "url": "https://openaipublic.azureedge.net/main/whisper/models/345ae4da62f9b3d59415adc60127b97c714f32e89e936602e85993674d08dcb1/medium.pt",
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}medium.pt",
            },
            {
                "name": "large-v1.pt",
            ...
            },
            {
                ...
                "dir": MODEL_DIR,
                "check": f"{MODEL_DIR}large-v3-turbo.pt",
            },
        ]
    ...
    def _add_section_header(self, text):
        ...
        lbl.setStyleSheet(
            "padding: 8px; font-weight: bold; font-size:14px; color: #fff; margin-top: 10px; border-radius: 4px;"
        )
