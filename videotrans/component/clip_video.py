...
class ClipVideoWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.output_mode.addItems(
            [tr("optionDefault"), tr("optionVideoOnly"), tr("optionAudioOnly"), tr("optionSeparateAV")]
        )
    ...
    def select_video(self):
        path, _ = QFileDialog.getOpenFileName(
            self, tr("selectVideo"), settings.get("last_opendir", ""), "Video Files (*.mp4 *.avi *.mkv)"
        )
        if path:
    ...
    def select_subtitle(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, tr("selectSubtitle"), settings.get("last_opendir", ""), "Subtitle Files (*.srt *.ass *.vtt)"
        )
        if path:
            ...
            for i, it in enumerate(self.subtitles):
                ...
                check = QCheckBox(
                    f"第{i+1}行 [{(it['end_time']-it['start_time'])/1000.0}s] {it['startraw']}->{it['endraw']}  {it['text']}"
                )
            ...
            self.progress_label.setPlainText(
                f"{tr('renderCompleteOutputTo')}:{output_folder}/{self.subtitle_name}-clip"
            )
...
class Worker(QThread):
    ...
    def __init__(self, *, parent: ClipVideoWindow, mode=None):
    ...
    def run(self):
        ...
        for line_num in self.parent.selected_lines:
            ...
            task = ClipTask(
                self.parent.video_path,
                sub,
                line_num,
                self.parent.subtitle_name,
                self.parent.signals,
                self.mode,
                video_info,
            )
