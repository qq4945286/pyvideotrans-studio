...
class ReDubb(QThread):
    ...
    def run(self):
        try:
            tts.run(queue_tts=[self.tts_dict], language=self.language, tts_type=self.tts_dict["tts_type"])
...
class EditDubbingResultDialog(QDialog):
    def __init__(self, parent=None, language=None, cache_folder: str = None):
        super().__init__()
        if DARK_STYLE_QSS:
        ...
        self.prompt_label = QLabel(
            tr(
                "You can check the voiceover here, or modify the text and re-encode the voiceover. Please stop the countdown before proceeding"
            )
        )
    ...
    def load_table(self):
        ...
        try:
            ...
            self.table.setHorizontalHeaderLabels(
                [
                    tr("Line"),
                    tr("Subtitles") + tr("Start Time"),
                    tr("Subtitles") + tr("End Time"),
                    tr("Dubbed Status"),
                    tr("Subtitle Text"),
                ]
            )
    ...
    def _custom_adjust(self, row, mode):
        ...
        ms, ok = QInputDialog.getInt(
            self, tr("Adjust time"), tr("Enter milliseconds (positive or negative):"), 0, -10000, 10000, 100
        )
        if ok:
