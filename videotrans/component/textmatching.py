...
TEXT_DB = {
    ...
    "label_paste_text": {
        "zh": "或者在下方粘贴文本 (优先使用输入框内容):",
        "en": "Or paste text below (Input box takes priority):",
    },
    "placeholder_text": {
        "zh": "在此处粘贴纯文本内容...如果不填则读取上方选择的文件",
        "en": "Paste plain text here... If empty, the file above will be used.",
    },
    ...
    "msg_provide_text": {
        "zh": "请提供文本内容（直接输入或选择文件）。",
        "en": "Please provide text content (Input or File).",
    },
    ...
    def run(self):
        try:
            ...
            tools.check_and_down_hf(
                self.model_name, FASTER_MODELS_DICT[self.model_name], self.local_dir, callback=self._progress_callback
            )
...
class TextmatchingWindow(QWidget):
    ...
    def select_audio(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self,
            tr("dialog_select_audio"),
            start_dir,
            "Audio Video Files (*.wav *.mp3 *.m4a *.flac *.aac *.mp4 *.mkv *.mov *.mpeg *.flv *.avi *.mpeg *.wmv *.wma *.ogg *.webm)",
        )
