...
class SetThreadProxy(QDialog):
    def __init__(self, parent=None):
        ...
        help_btn.setStyleSheet("background-color: transparent;color:#1A72BB;text-decoration:underline;")
        ...
        if proxy:
            ...
            if not re.match(r"^(http|sock)(s|5)?://(\d+\.){3}\d+:\d+", proxy, re.I):
                question = tools.show_popup(
                    ...
                    tr(
                        "The network proxy address you fill in seems to be incorrect, the general proxy/vpn format is http://127.0.0.1:port, if you do not know what is the proxy please do not fill in arbitrarily, ChatGPT and other api address please fill in the menu - settings - corresponding configuration. If you confirm that the proxy address is correct, please click Yes to continue."
                    ),
                )
