...
class LocalLLM(BaseTrans):
    ...
    def __post_init__(self):
        ...
        self.prompt = tools.get_prompt(ainame="localllm", aisendsrt=self.aisendsrt).replace(
            "{lang}", self.target_language_name
        )
    ...
    def _item_task(self, data: Union[List[str], str]) -> str:
        if self._exit():
            return
        model = OpenAI(
            api_key=params.get("localllm_key", ""),
        ...
        message = [
            ...
            {
                "role": "user",
                "content": self.prompt.replace("{batch_input}", f"{text}").replace(
                    "{context_block}", self.full_origin_subtitles
                ),
            },
        ]
        ...
        response = model.chat.completions.create(
            ...
            max_tokens=int(params.get("localllm_max_token")) if params.get("localllm_max_token") else 4096,
        ...
        match = re.search(
            r"<TRANSLATE_TEXT>(.*?)</TRANSLATE_TEXT>", re.sub(r"<think>(.*?)</think>", "", result, flags=re.I | re.S)
        )
        if match:
