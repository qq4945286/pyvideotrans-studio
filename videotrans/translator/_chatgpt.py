...
class ChatGPT(BaseTrans):
    ...
    def __post_init__(self):
        ...
        self.prompt = tools.get_prompt(ainame="chatgpt", aisendsrt=self.aisendsrt).replace(
    ...
    def llm_segment(self, srt_list, ai_type="openai"):
        ...
        def _send(srt):
            ...
            match = re.search(
                r"<SRT>(.*?)</SRT>", re.sub(r"<think>(.*?)</think>", "", result, flags=re.I | re.S), re.S | re.I
            )
    ...
    def _item_task(self, data: Union[List[str], str]) -> str:
        ...
        model = OpenAI(
            api_key=params.get("chatgpt_key", ""),
