...
LANG_CODE = {
    ...
    "en": ["en", "eng", "en", "EN-US", "en", "en", "en", "English", "en", "English", "en"],  # m2m100
    "fr": ["fr", "fre", "fra", "FR", "fr", "fr", "fr", "French", "fr", "French", "fr"],  # m2m100
    "de": ["de", "ger", "de", "DE", "de", "de", "de", "German", "de", "German", "de"],  # m2m100
    "ja": ["ja", "jpn", "jp", "JA", "ja", "ja", "ja", "Japanese", "ja", "Japanese", "ja"],  # m2m100
    "ko": ["ko", "kor", "kor", "KO", "ko", "ko", "ko", "Korean", "ko", "Korean", "ko"],  # m2m100
    "ru": ["ru", "rus", "ru", "RU", "ru", "ru", "ru", "Russian", "ru", "Russian", "ru"],  # m2m100
    "es": ["es", "spa", "spa", "ES", "es", "es", "es", "Spanish", "es", "Spanish", "es"],  # m2m100
    "th": ["th", "tha", "th", "No", "th", "th", "th", "Thai", "th", "Thai", "th"],  # m2m100
    "it": ["it", "ita", "it", "IT", "it", "it", "it", "Italian", "it", "Italian", "it"],  # m2m100
    "el": [
        "el",  # google
    ...
    "pt": ["pt", "por", "pt", "PT-PT", "PT-PT", "pt", "pt", "Portuguese", "pt", "Portuguese", "pt"],  # pt-PT  # m2m100
    "vi": ["vi", "vie", "vie", "vi", "vi", "vi", "vi", "Vietnamese", "vi", "Vietnamese", "vi"],  # m2m100
    "ar": ["ar", "are", "ara", "AR", "ar", "ar", "ar", "Arabic", "ar", "Arabic", "ar"],  # m2m100
    "tr": ["tr", "tur", "tr", "TR", "tr", "tr", "tr", "Turkish", "tr", "Turkish", "tr"],  # m2m100
    "hi": ["hi", "hin", "hi", "No", "hi", "hi", "hi", "Hindi", "hi", "Hindi", "hi"],  # m2m100
    "hu": ["hu", "hun", "hu", "HU", "No", "hu", "hu", "Hungarian", "hu", "Hungarian", "hu"],  # m2m100
    "uk": [
        "uk",
        "ukr",
        "ukr",  # 百度
    ...
    "id": ["id", "ind", "id", "ID", "id", "id", "id", "Indonesian", "id", "Indonesian", "id"],  # m2m100
    "ms": ["ms", "may", "may", "No", "ms", "ms", "ms", "Malay", "ms", "Malay", "ms"],  # m2m100
    "kk": ["kk", "kaz", "No", "No", "No", "No", "kk", "Kazakh", "kk", "Kazakh", "kk"],  # m2m100
    "cs": ["cs", "ces", "cs", "CS", "No", "cs", "cs", "Czech", "cs", "Czech", "cs"],  # m2m100
    "pl": ["pl", "pol", "pl", "PL", "No", "pl", "pl", "Polish", "pl", "Polish", "pl"],  # m2m100
    "nl": [
        "nl",  # google通道
        "dut",  # 字幕嵌入语言
        "nl",  # 百度通道
...
def is_allow_translate(*, translate_type=None, show_target=None, only_key=False, return_str=False):
    ...
    if translate_type == QWENMT_INDEX and not params.get("qwenmt_key", ""):
        ...
        qwenmt.openwin()
        return False
    if translate_type == AZUREGPT_INDEX and (not params.get("azure_key", "") or not params.get("azure_api", "")):
        if return_str:
    ...
    if translate_type == BAIDU_INDEX and (not params.get("baidu_appid", "") or not params.get("baidu_miyue", "")):
        if return_str:
    ...
    if translate_type == TENCENT_INDEX and (
        ...
        tencent.openwin()
        return False
    if translate_type == ALI_INDEX and (not params.get("ali_id", "") or not params.get("ali_key", "")):
        if return_str:
...
def get_audio_code(*, show_source=None):
    if not show_source or show_source in ["auto", "-"]:
        return "auto"
    source_list = (
        LANG_CODE[show_source] if show_source in LANG_CODE else LANG_CODE.get(LANGNAME_DICT_REV.get(show_source))
    )
...
def run(
    *, translate_type=0, text_list=None, is_test=False, source_code=None, target_code=None, uuid=None
) -> Union[List, str, None]:
