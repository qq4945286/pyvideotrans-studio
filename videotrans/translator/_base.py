...
class BaseTrans(BaseCon):
    ...
    def __post_init__(self):
        ...
        if self.aisendsrt and settings.get("aitrans_context"):
            self.full_origin_subtitles = _GLOBAL_CONTEXT.replace(
    ...
    def _get_key(self, it):
        ...
        key_str = (
            f"{self.translate_type}-{self.api_url}-{self.aisendsrt}-{self.model_name}-{self.source_code}-{self.target_code}-"
            + (it if isinstance(it, str) else json.dumps(it))
        )
