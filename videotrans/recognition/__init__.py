...
HUGGINGFACE_ASR_MODELS = {
    ...
    "Yehor/whisper-large-v3-turbo-quantized-uk": ["uk"],
...
def get_moonshine_models(lang_code: str = "") -> list:
    """返回模型列表。已知语言过滤+语言后缀，未知则显示全部"""
    lang = (lang_code or "")[:2].lower()
    if lang in _MOONSHINE_LANG_ARCHS:
        d = _MOONSHINE_LANG_DISPLAY[lang]
        return [f"{_MOONSHINE_ARCH_NAMES[a]}({d})" for a in sorted(_MOONSHINE_LANG_ARCHS[lang])]
    # 未知语言（Auto Detect等），显示全部
    items = []
    for lc in sorted(_MOONSHINE_LANG_ARCHS):
        d = _MOONSHINE_LANG_DISPLAY[lc]
        items += [f"{_MOONSHINE_ARCH_NAMES[a]}({d})" for a in sorted(_MOONSHINE_LANG_ARCHS[lc])]
    return items
...
def is_allow_lang(langcode: str = None, recogn_type: int = None, model_name=None):
    ...
    if recogn_type == HUGGINGFACE_ASR and HUGGINGFACE_ASR_MODELS.get(model_name):
        ...
        return True
    if recogn_type == MOONSHINE:
        lang = (langcode or "en")[:2].lower()
        if lang not in MOONSHINE_SUPPORTED_LANGS:
            return tr("Moonshine-voice only supports: ") + ", ".join(sorted(MOONSHINE_SUPPORTED_LANGS))
        return True
...
def is_input_api(recogn_type: int = None, return_str=False):
    from videotrans.winform import (
        recognapi as recognapi_win,
        openairecognapi as openairecognapi_win,
        doubao as doubao_win,
        sttapi as sttapi_win,
        deepgram as deepgram_win,
        gemini as gemini_win,
        ai302,
        parakeet as parakeet_win,
        qwenmt as qwenmt_win,
        zijierecognmodel as zijierecogn_win,
    )
...
def run(
    *,
    ...
    # ROCm 环境下 faster-whisper 不支持 GPU，自动切换到 openai-whisper 以使用 GPU 加速
    if is_cuda and recogn_type == FASTER_WHISPER:
        import torch

        if torch.version.hip is not None:
            logger.warning("ROCm 检测: faster-whisper 不支持 GPU，自动切换到 openai-whisper 使用 GPU 加速")
            recogn_type = OPENAI_WHISPER
    ...
    if recogn_type == MOONSHINE:
        from videotrans.recognition._moonshine import MoonshineRecogn

        return MoonshineRecogn(**kwargs).run()
