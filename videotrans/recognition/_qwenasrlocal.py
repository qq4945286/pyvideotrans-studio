...
class QwenasrlocalRecogn(BaseRecogn):
    ...
    def _download(self):
        if defaulelang == "zh":
            tools.check_and_down_ms(
                f"Qwen/Qwen3-ASR-{self.model_name}",
            ...
            tools.check_and_down_hf(
                model_id=f"Qwen3-ASR-{self.model_name}",
    ...
    def segmentation_asr_data(
        self, asr_data, min_duration=1.0, max_pref_duration=6.0, max_hard_duration=8.0, silence_threshold=0.4
    ):
        """
        将ASR字词级数据重组为1-6秒的句子。
        ...
        def is_cjk(char):
            ...
            return 0x4E00 <= code <= 0x9FFF or 0x3040 <= code <= 0x309F or 0x30A0 <= code <= 0x30FF
