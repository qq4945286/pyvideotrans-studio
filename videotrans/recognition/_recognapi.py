...
class APIRecogn(BaseRecogn):
    ...
    def _exec(self) -> Union[List[Dict], None]:
        ...
        if "application/json" in content_type:
            ...
            self._signal(text=tools.get_srt_from_list(res["data"]), type="replace_subtitle")
            return res["data"]
    ...
    def _whisperzero(self):
        ...
        with open(self.audio_file, "rb") as f:
            audio_file = f.read()
        files = {"audio": (self.audio_file, audio_file, "audio/wav")}  # Content-Type 音频类型，有些API需要特别指定

        response = requests.post("https://api.gladia.io/v2/upload", files=files, headers={"x-gladia-key": api_key})
        ...
        def _process_chunk_result(raw_text, time_offset_ms, start_line_index):
            ...
            try:
                ...
                try:
                    ...
                    context = {"null": None, "true": True, "false": False, "__builtins__": None}
        ...
        for i, start_ms in enumerate(range(0, total_duration, CHUNK_DURATION_MS)):
            ...
            try:
                ...
                chunk_data, chunk_spk = _process_chunk_result(
                    result[0], time_offset_ms=start_ms, start_line_index=current_line
                )
            ...
            try:
                ...
                if final_speaker_list:
                    Path(f"{self.cache_folder}/speaker.json").write_text(
                        json.dumps(final_speaker_list), encoding="utf-8"
                    )
            except Exception as e:
