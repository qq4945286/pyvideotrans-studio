...
class BaseRecogn(BaseCon):
    ...
    def run(self) -> Union[List[Dict], None]:
        ...
        try:
            ...
            if not res:
                raise RuntimeError(
                    tr(
                        "No speech was detected, please make sure there is human speech in the selected audio/video and that the language is the same as the selected one."
                    )
                )
            ...
            if (
                self.llm_post
                or not settings.get("merge_short_sub", True)
                or (self.recogn_type < 2 and not settings.get("whisper_prepare"))
            ):
    ...
    def _fix_post(self, srt_list):
        ...
        if (
            post_srt_raws[0]["end_time"] - post_srt_raws[0]["start_time"] < min_speech
        ...
        if (
            post_srt_raws[-1]["end_time"] - post_srt_raws[-1]["start_time"] < min_speech
        ...
        for i, it in enumerate(post_srt_raws):
            ...
            post_srt_raws[i + 1]["text"] = (
                it["text"][-len(t[-1]) :] + self.join_word_flag + post_srt_raws[i + 1]["text"]
            )
