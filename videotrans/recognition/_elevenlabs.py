...
class ElevenLabsRecogn(BaseRecogn):
    ...
    def _exec(self) -> Union[List[Dict], None]:
        ...
        client = ElevenLabs(
            api_key=params.get("elevenlabstts_key", ""), httpx_client=httpx.Client(proxy=self.proxy_str)
        )
        ...
        if language_code:
            res = client.speech_to_text.convert(
                model_id="scribe_v2", file=file_object, language_code=language_code, diarize=True
            )
        else:
            res = client.speech_to_text.convert(model_id="scribe_v1", file=file_object, diarize=True)
        last_tmp = None
        ...
        for it in res.words:
            ...
            if not last_tmp:
                if not text.strip():
                    continue
                last_tmp = {"line": len(raws) + 1, "text": text, "start_time": st, "end_time": end, "spk": spk}
                continue
            ...
            if spk != last_tmp["spk"]:
                last_tmp["time"] = (
                    tools.ms_to_time_string(ms=last_tmp["start_time"])
                    + " --> "
                    + tools.ms_to_time_string(ms=last_tmp["end_time"])
                )
            ...
            if (diff_prev >= 200 or segment_time >= 500) and isflag:
                ...
                if text[0] in self.flag:
                    ...
                    if last_tmp["text"].strip():
                        raws.append(last_tmp)
                    last_tmp = {"line": len(raws) + 1, "text": text[1:], "start_time": st, "end_time": end, "spk": spk}
                    continue

                last_tmp["end_time"] = end
                last_tmp["text"] += text
                last_tmp["time"] = (
                    tools.ms_to_time_string(ms=last_tmp["start_time"]) + " --> " + tools.ms_to_time_string(ms=end)
                )
