...
def _handle_connection_error_detail(error, lang):
    ...
    if "dns" in error_str or "name or service not known" in error_str:
        ...
        base_message = "连接被重置，网络可能不稳定" if lang == "zh" else "Connection reset, network may be unstable"
    elif "timeout" in error_str or "timed out" in error_str:
        base_message = (
            "连接超时，请检查网络连接是否稳定" if lang == "zh" else "Connection timeout, please check network stability"
        )
        ...
        base_message = "网络连接失败" if lang == "zh" else "Network connection failed"
...
def get_msg_from_except(ex):
    ...
    exception_handlers = {
        ...
        AuthenticationError: lambda e: (
            f"API密钥错误，请检查密钥是否正确 {e.body.get('message')}" if lang == "zh" else e.body.get("message")
        ),
        PermissionDeniedError: lambda e: (
            f"当前密钥没有访问权限，请检查权限设置 {e.body.get('message')}" if lang == "zh" else e.message
        ),
        # === 频率限制 ===
        RateLimitError: lambda e: (
            f"请求过于频繁或余额不足：{e.body.get('message')}" if lang == "zh" else e.body.get("message")
        ),
        # === 资源不存在问题 ===
        # === 请求参数问题 ===
        # === 服务端问题 ===
        (InternalServerError, NotFoundError, BadRequestError, APIConnectionError, APIError): lambda e: (
            e.body.get("message") if hasattr(e, "body") and hasattr(e.body, "get") else str(e)
        ),
        ...
        ContentFilterFinishReasonError: lambda e: (
            f"内容触发AI风控被过滤 {e}" if lang == "zh" else f"Content triggers AI risk control and is filtered\n{e}"
        ),
        # === 配置和地址问题 ===
        (TooManyRedirects, MissingSchema, InvalidSchema, InvalidURL): lambda e: (
            f"请求地址格式不正确，请检查配置 {e.message}"
            if lang == "zh"
        ...
        Timeout: lambda e: (_handle_connection_error_detail(e, lang)),
        HTTPError: lambda e: f"{e}",
        RetryError: lambda e: (
            "重试多次后仍然失败，请检查网络连接或服务状态"
            if lang == "zh"
        ...
        ConnectionRefusedError: lambda e: (_handle_connection_error_detail(e, lang)),
        ConnectionResetError: lambda e: (_handle_connection_error_detail(e, lang)),
        ConnectionAbortedError: lambda e: (
            "连接意外中断，请检查网络稳定性"
            if lang == "zh"
            else "Connection aborted unexpectedly, check network stability"
        ),
        (ReqConnectionError, ConnectionError): lambda e: (_handle_connection_error_detail(e, lang)),
        requests.exceptions.RequestException: lambda e: f"{e}",
        RuntimeError: lambda e: (f"{e}" if lang == "zh" else f"{e}"),
        FileNotFoundError: lambda e: (
            f"文件不存在：{getattr(e, 'filename', '')}"
            if lang == "zh"
            else f"File not found: {getattr(e, 'filename', '')}"
        ),
        PermissionError: lambda e: (
            f"权限不足，无法访问：{getattr(e, 'filename', '')}"
            if lang == "zh"
            else f"Permission denied: {getattr(e, 'filename', '')}"
        ),
        FileExistsError: lambda e: (
            f"文件已存在：{getattr(e, 'filename', '')}"
            if lang == "zh"
            else f"File already exists: {getattr(e, 'filename', '')}"
        ),
        # === 操作系统错误 ===
        OSError: lambda e: (
            f"系统错误 ({e.errno})：{e.strerror}" if lang == "zh" else f"System Error ({e.errno}): {e.strerror}"
        ),
        # === 数据处理错误 ===
        KeyError: lambda e: (f"处理数据时缺少必需的键：{e}" if lang == "zh" else f"{e}"),
        IndexError: lambda e: (f"处理列表或序列时索引越界:{e}" if lang == "zh" else f"{e}"),
        LookupError: lambda e: (f"查找错误，指定的键或索引不存在:{e}" if lang == "zh" else f"{e}"),
        UnicodeDecodeError: lambda e: (
            f"文件或数据解码失败，编码格式错误：{e.reason}" if lang == "zh" else f" {e.reason}"
        ),
        ValueError: lambda e: (f"无效的值或参数：{e}" if lang == "zh" else f"{e}"),
        # === 程序内部错误 ===
        AttributeError: lambda e: (f"程序内部错误：{e}" if lang == "zh" else f"{e}"),
        NameError: lambda e: (f"程序内部错误：未定义的变量 '{e.name}'" if lang == "zh" else f"{e}"),
        TypeError: lambda e: (f"程序内部错误：{e}" if lang == "zh" else f"{e}"),
        RecursionError: lambda e: (f"程序内部错误：发生无限递归:{e}" if lang == "zh" else f"{e}"),
        ZeroDivisionError: lambda e: (f"算术错误：除数为零:{e}" if lang == "zh" else f"{e}"),
        OverflowError: lambda e: (f"算术错误：数值超出最大限制:{e}" if lang == "zh" else f"{e}"),
        BrokenPipeError: lambda e: (
            "连接管道损坏，请检查网络连接" if lang == "zh" else "Broken pipe error, check network connection"
        ),
    }
    ...
    if hasattr(ex, "error") and ex.error:
        ...
        return f"错误详情：{error_msg}" if lang == "zh" else f"Error details: {error_msg}"
