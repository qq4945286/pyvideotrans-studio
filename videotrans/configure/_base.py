...
from videotrans.util.gpus import get_cudaX, is_rocm
...
class BaseCon:
    ...
    def _external_cmd_with_wrapper(self, cmd_list=None):
        ...
        try:
            ...
            if os.name == "nt" and IS_FROZEN:
                raise RuntimeError(
                    tr(
                        "Currently Faster-Whisper-XXL cannot be used in the packaged version. Please deploy the source code or use Faster-Whisper-XXL transcription separately."
                    )
                )
    ...
    def _base64_to_audio(self, encoded_str: str, output_path: str) -> None:
        ...
        if encoded_str.startswith("data:audio/"):
            ...
            support_format = {"mpeg": "mp3", "wav": "wav", "ogg": "ogg", "aac": "aac"}
            ...
            if base64data_ext and base64data_ext != output_ext:
                ...
                tools.runffmpeg(["-y", "-i", output_path + f".{base64data_ext}", "-b:a", "128k", output_path])
                return
        # 将base64编码的字符串解码为字节
        wav_bytes = base64.b64decode(encoded_str)
        # 将解码后的字节写入文件
        ...
        try:
            ...
            if is_cuda:
                ...
                if device_index == -1:
                    ...
                    logger.error(f"已启用{backend}但未检测到可用显卡，强制使用CPU")
                kwargs["device_index"] = max(device_index, 0)
            future = (
                GlobalProcessManager.submit_task_cpu(callback, **kwargs)
                if not is_cuda
                else GlobalProcessManager.submit_task_gpu(callback, **kwargs)
            )
            _rs = future.result()
