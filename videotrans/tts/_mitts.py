...
class MITTS(BaseTTS):
    ...
    def __post_init__(self):
        ...
        self.client = OpenAI(api_key=params.get("mitts_key", ""), base_url="https://api.xiaomimimo.com/v1")
    ...
    def _item_task(self, data_item: dict = None, idx: int = -1):
        ...
        if idx > -1:
            refer_text = " ".join(
                [it["text"] for it in self.queue_tts[max(0, idx - 5) : min(idx + 6, len(self.queue_tts))]]
            )
        post_message = [
            {"role": "user", "content": refer_text},
            {"role": "assistant", "content": data_item.get("text")},
        ]
        ...
        def _run():
            ...
            try:
                completion = self.client.chat.completions.create(
                    model="mimo-v2-tts", messages=post_message, audio={"format": "wav", "voice": data_item["role"]}
                )
