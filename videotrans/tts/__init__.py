...
def is_allow_lang(langcode: str = None, tts_type: int = None):
    ...
    if tts_type == QWEN3LOCAL_TTS and langcode[:2] not in [
        "zh",
        "ja",
        "ko",
        "en",
        ...
        return (
            ...
            + " "
            + tr("Only support")
            + tr(["zh", "ja", "ko", "en", "yu", "de", "fr", "ru", "pt", "es", "it"])
        )
...
def is_input_api(tts_type: int = None, return_str=False):
    ...
    if tts_type == CHATTERBOX_TTS and not params.get("chatterbox_url", ""):
        ...
        return False
    if (
        tts_type == COSYVOICE_TTS
        and params.get("cosyvoice_url", "").replace("http://", "").replace("https://", "").strip() == ""
    ):
        if return_str:
    ...
    if tts_type == CHATTTS and not params.get("chattts_api", ""):
        ...
        chattts_win.openwin()
        return False
    if tts_type == AZURE_TTS and (not params.get("azure_speech_key", "") or not params.get("azure_speech_region", "")):
        if return_str:
    ...
    if tts_type == DOUBAO_TTS and (
        not params.get("volcenginetts_appid", "")
        or not params.get("volcenginetts_access", "")
        or not params.get("volcenginetts_cluster", "")
    ):
        if return_str:
