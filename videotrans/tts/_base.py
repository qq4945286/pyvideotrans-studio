...
class BaseTTS(BaseCon):
    ...
    def _cleantts(self):
        ...
        if "volume" in self.queue_tts[0]:
            self.volume = str(self.queue_tts[0]["volume"])
        if "rate" in self.queue_tts[0]:
            self.rate = str(self.queue_tts[0]["rate"])
        if "pitch" in self.queue_tts[0]:
            self.pitch = str(self.queue_tts[0]["pitch"])
    ...
    def _padforaudio(self, duration=1500):
        ...
        silent_segment = AudioSegment.silent(duration=duration).set_channels(1).set_frame_rate(16000)
