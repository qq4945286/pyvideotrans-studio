    ...
    def _download(self):
        if defaulelang == "zh":
            tools.check_and_down_ms(
                f"Qwen/Qwen3-TTS-12Hz-{self.model_name}-Base",
                callback=self._process_callback,
                local_dir=f"{ROOT_DIR}/models/models--Qwen--Qwen3-TTS-12Hz-{self.model_name}-Base",
            )
            tools.check_and_down_ms(
                f"Qwen/Qwen3-TTS-12Hz-{self.model_name}-CustomVoice",
                callback=self._process_callback,
                local_dir=f"{ROOT_DIR}/models/models--Qwen--Qwen3-TTS-12Hz-{self.model_name}-CustomVoice",
            )
        else:
            tools.check_and_down_hf(
                model_id=f"Qwen3-TTS-12Hz-{self.model_name}-Base",
                repo_id=f"Qwen/Qwen3-TTS-12Hz-{self.model_name}-Base",
                local_dir=f"{ROOT_DIR}/models/models--Qwen--Qwen3-TTS-12Hz-{self.model_name}-Base",
                callback=self._process_callback,
            )
            tools.check_and_down_hf(
                model_id=f"Qwen3-TTS-12Hz-{self.model_name}-CustomVoice",
                repo_id=f"Qwen/Qwen3-TTS-12Hz-{self.model_name}-CustomVoice",
                local_dir=f"{ROOT_DIR}/models/models--Qwen--Qwen3-TTS-12Hz-{self.model_name}-CustomVoice",
                callback=self._process_callback,
            )
