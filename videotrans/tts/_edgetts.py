...
class EdgeTTS(BaseTTS):
    def __post_init__(self):
        ...
        self.useproxy = (
            None if not self.proxy_str or Path(f"{ROOT_DIR}/edgetts-noproxy.txt").exists() else self.proxy_str
        )
            ...
            async with semaphore:
                ...
                for attempt in range(RETRY_NUMS + 1):
                    ...
                    try:
                        ...
                        await asyncio.wait_for(communicate.save(item["filename"] + ".mp3"), timeout=SAVE_TIMEOUT)

                        if self._stop_event.is_set():
                            return
                        loop = asyncio.get_running_loop()
                        signal_with_args = functools.partial(
                            self._signal, text=f'{tr("kaishipeiyin")} {msg}[{self.ends_counter + 1}/{total_tasks}]'
                        )

                        try:
                            await asyncio.wait_for(loop.run_in_executor(None, signal_with_args), timeout=SIGNAL_TIMEOUT)
                        ...
                        self.error = (
                            ...
                            else f'proxy={self.useproxy}, {tr("Please turn off the clear proxy and try again")}:{e}'
                        )
                        # 强制禁用代理重试
                        self.useproxy = None
                        if attempt < RETRY_NUMS:
    ...
    async def _exec(self) -> None:
        ...
        worker_tasks = [
            asyncio.create_task(self._create_audio_with_retry(item, i, total_tasks, semaphore))
