...
class AzureTTS(BaseTTS):
    ...
    def _item_task_pl(self, items: list = None):
        ...
        def _run():
            ...
            speech_config = speechsdk.SpeechConfig(
                subscription=params.get("azure_speech_key", ""), region=params.get("azure_speech_region", "")
            )
            ...
            ssml = """<speak version='1.0' xml:lang='{}' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts'>
                ...
                self.language,
                tools.get_azure_rolelist(self.language.split("-")[0], items[0]["role"]),
                self.rate,
                self.pitch,
                self.volume,
                text_xml,
            ...
            def bookmark_reached(event):
                bookmarks.append({"time": event.audio_offset / 10000})  # 转换为毫秒
            ...
            if speech_synthesis_result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                ...
                for i, it in enumerate(bookmarks):
                    if i >= length or not tools.vail_file(filename):
                        continue
                    cmd = ["-y", "-i", filename, "-ss", str(it["time"] / 1000)]
                    if i < length - 1:
                        cmd += ["-t", str((bookmarks[i + 1]["time"] - it["time"]) / 1000)]
    ...
    def _item_task(self, data_item, idx: int = -1):
        ...
        def _run():
            ...
            speech_synthesizer = speechsdk.SpeechSynthesizer(speech_config=speech_config, audio_config=audio_config)
            text_xml = (
                f"<prosody rate='{self.rate}' pitch='{self.pitch}' volume='{self.volume}'>{data_item['text']}</prosody>"
            )
            ssml = """<speak version='1.0' xml:lang='{}' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts'>
                                    <voice name='{}'>
                                        <prosody rate="{}" pitch='{}'  volume='{}'>
                ...
                self.language,
                tools.get_azure_rolelist(self.language.split("-")[0], data_item["role"]),
                self.rate,
                self.pitch,
                self.volume,
                text_xml,
