...
class F5TTS(BaseTTS):
    ...
    def _item_task_v1(self, data_item: Union[Dict, List, None]):
        ...
        if role == "clone":
            ...
            data["ref_text"] = (data_item.get("ref_text") or "").strip()
        else:
    ...
    def _item_task_index(self, data_item: Union[Dict, List, None]):
        ...
        kw = {"prompt": handle_file(data["ref_wav"]), "text": text, "api_name": "/gen_single"}
        # 0=v1 1=v2
    ...
    def _item_task_voxcpm(self, data_item: Union[Dict, List, None]):
        ...
        params_version = {"do_normalize": True, "denoise": True, "api_name": "/generate"}
    ...
    def _item_task(self, data_item: Union[Dict, List, None], idx: int = -1):
        ...
        def _run():
            ...
            try:
                self.client = Client(
                    self.api_url, httpx_kwargs={"timeout": 7200, "proxy": self.proxy_str}, ssl_verify=False
                )
            except ValueError as e:
                if "api_name" in str(e):
                    raise StopRetry(
                        f"api_name名称不正确，请确保使用该TTS的官方源码部署\F5-TTS: https://github.com/SWivid/F5-TTS\nIndex-TTS: https://github.com/index-tts/index-tts\nSpark-TTS: https://github.com/SparkAudio/Spark-TTS\nVoxCPM-TTS: https://github.com/OpenBMB/VoxCPM\nDia-TTS: https://github.com/nari-labs/dia"
                    )
                raise
            except Exception as e:
                raise StopRetry(f"{e}")
