...
def runffmpeg(arg, *, noextname=None, uuid=None, force_cpu=True, cmd_dir=None):
    ...
    try:
        ...
        logger.warning(
            f"FFmpeg 命令执行失败 (force_cpu={force_cpu})。\n命令: {' '.join(cmd)}\n错误: {error_message} {e.stdout}"
        )
...
def get_video_codec(compat=None) -> str:
    ...
    def test_encoder_internal(encoder_to_test: str, timeout: int = 10) -> bool:
        ...
        # VAAPI on Linux needs device init and format conversion
        if encoder_to_test.endswith("_vaapi") and plat == "Linux":
            import glob
                ...
                if encoder_suffix == "nvenc":
                    try:
                        if not torch.cuda.is_available():
                            from videotrans.util.gpus import is_rocm

                            backend = "ROCm" if is_rocm() else "CUDA"
                            logger.debug(f"{backend} 不可用，跳过 nvenc 测试。")
...
def get_video_info(mp4_file, *, video_fps=False, video_scale=False, video_time=False, get_codec=False):
    ...
    try:
        out_json = runffprobe(["-v", "quiet", "-print_format", "json", "-show_format", "-show_streams", mp4_file])
        if not out_json:
            raise RuntimeError(
    ...
    if "streams" not in out or not out["streams"]:
        raise RuntimeError(
            tr(
                "The original file {} does not contain any audio or video data. The file may be damaged. Please confirm that the file can be played.",
                mp4_file,
            )
        )
...
def wav2m4a(wavfile, m4afile, extra=None):
    cmd = ["-y", "-i", Path(wavfile).as_posix(), "-c:a", "aac", "-b:a", "128k", Path(m4afile).as_posix()]
    if extra:
...
def create_concat_txt(filelist, concat_txt=None):
    ...
    for it in filelist:
        ...
        if not path_obj.exists() or path_obj.stat().st_size == 0:
            continue
        # 使用绝对路径，保证 ffmpeg 在任何工作目录下都能找到
        txt.append(f"file '{path_obj.resolve().as_posix()}'")
    if not txt:
...
def precise_speed_up_audio(*, file_path=None, out=None, target_duration_ms=None):
    ...
    ext = _Path(file_path).suffix[1:].lower()
    out_ext = ext
    if out:
        out_ext = _Path(out).suffix[1:].lower()
    ...
    cmd = [
        ...
        "-t",
        f"{target_duration_ms/1000.0}",  # 强制裁剪到目标时长，防止精度误差
        "-ar",
        "48000",
        "-ac",
        "2",
        "-c:a",
        codecs.get(out_ext, "pcm_s16le"),
        out,
    ]
    try:
...
def remove_silence_wav(audio_file):
    ...
    nonsilent_chunks = detect_nonsilent(
        audio, min_silence_len=min_silence_len, silence_thresh=silence_threshold, seek_step=1
    )
...
def remove_silence_from_end(input_file_path, is_start=True):
    ...
    nonsilent_chunks = detect_nonsilent(audio, min_silence_len=10, silence_thresh=audio.dBFS - 20)
