...
def getset_gpu(force_cpu=False) -> int:
    ...
    if app_cfg.NVIDIA_GPU_NUMS > 0:
        if is_rocm():
            for i in range(app_cfg.NVIDIA_GPU_NUMS):
                props = torch.cuda.get_device_properties(i)
                print(f"ROCm GPU detected: {props.name}, {props.total_memory / 1024**3:.1f}GB")
        else:
...
def get_cudaX() -> int:
    ...
    try:
        ...
        logger.exception(f"获取当前可用{backend}显卡索引失败,返回第0块显卡:{e}", exc_info=True)
        return 0
