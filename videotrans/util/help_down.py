    ...
    try:
        ...
        try:
            huggingface_hub.snapshot_download(
                repo_id=repo_id, local_dir=local_dir, etag_timeout=5, local_files_only=True
            )
        ...
        msg = (
            f"下载模型失败，你可以打开以下网址，将所有文件下载到\n {local_dir} 文件夹内\n"
            if defaulelang == "zh"
            else f"The model download failed. You can try opening the following URL and downloading all files to the {local_dir} folder."
        )
        raise RuntimeError(f"{msg}\n[https://huggingface.co/{repo_id}/tree/main]\n{e}")
    else:
        junk_paths = [".cache", "blobs", "refs", "snapshots", ".no_exist"]
...
def down_file_from_hf(local_dir, urls=None, callback=None) -> bool:
    ...
    for index, url in enumerate(urls):
        try:
            ...
            raise RuntimeError(tr("downloading all files", local_dir) + f"\n[https://huggingface.co{url}]\n\n{e}")
    return True
