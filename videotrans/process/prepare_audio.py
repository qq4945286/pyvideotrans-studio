...
def cam_speakers(*, input_file, subtitles, num_speakers=-1, is_cuda=False, logs_file=None, device_index=0):
    import torch, os, shutil, time
    from modelscope.pipelines import pipeline
    import platform
    ...
    try:
        ...
        result = (
            ans(input_file, oracle_num=num_speakers, ignore_errors=True)
            if num_speakers > 1
            else ans(input_file, ignore_errors=True)
        )
...
def pyannote_speakers(*, input_file, subtitles, num_speakers=-1, is_cuda=False, logs_file=None, device_index=0):
...
def reverb_speakers(*, input_file, subtitles, num_speakers=-1, is_cuda=False, logs_file=None, device_index=0):
    import torch, pyannote.audio, torchaudio, os, shutil, time
...
# 内置中英文说话人分离模型
# 仅使用cpu，不使用gpu
def built_speakers(*, input_file, subtitles, num_speakers=-1, language="zh", logs_file=None, is_cuda=False):
    from pathlib import Path
    import torch
    ...
    def init_speaker_diarization(language, num_speakers=-1):
        ...
        embedding_extractor_model = (
            f"{ROOT_DIR}/models/onnx/3dspeaker_speech_eres2net_large_sv_zh-cn_3dspeaker_16k.onnx"
            if language == "zh"
            else f"{ROOT_DIR}/models/onnx/nemo_en_titanet_small.onnx"
        )
        ...
        _cf = sherpa_onnx.OfflineSpeakerDiarizationConfig(
            ...
            clustering=sherpa_onnx.FastClusteringConfig(num_clusters=num_speakers, threshold=0.5),  # cluster_threshold
            min_duration_on=0.3,
            min_duration_off=0.5,
        )
        if not _cf.validate():
            raise RuntimeError("Please check your config and make sure all required files exist")
    ...
    def _get_diariz():
        ...
        if sample_rate != sd.sample_rate:
            raise RuntimeError(f"Expected samples rate: {sd.sample_rate}, given: {sample_rate}")
