...
def openai_whisper(
    *,
    ...
    if not Path(f"{ROOT_DIR}/models/{model_name}.pt").exists():
        msg = (
            f"模型 {model_name} 不存在，将自动下载 "
            if defaulelang == "zh"
            else f"Model {model_name} does not exist and will be automatically downloaded"
        )
    else:
    ...
    try:
        ...
        model = whisper.load_model(
            model_name, device=f"cuda:{device_index}" if is_cuda else "cpu", download_root=ROOT_DIR + "/models"
        )
        ...
        if speech_timestamps:
            ...
            for segment in result["segments"]:
                ...
                tmp = {"text": text, "start_time": s, "end_time": e}
            ...
            logger.debug(
                f"openai-whisper模式下，预先使用VAD分割音频，直接使用{model_name}模型返回的各个片段音频的文字结果"
            )
        else:
                ...
                texts.append(
                    {
                        ...
                        "end": segment["end"],
                        "words": [
                            {"word": it["word"], "start": it["start"], "end": it["end"]} for it in segment["words"]
                        ],
                    }
                )
...
def _resegment(texts, language, max_speech_ms):
    ...
    for segment in texts:
        ...
        if seg_duration <= max_speech_ms or not words:
            final_segments.append({"text": segment.get("text", "").strip(), "start": seg_start_ms, "end": seg_end_ms})
            continue
        ...
        for w in words:
            ...
            if should_split:
                ...
                final_segments.append(
                    {"text": build_text(current_chunk), "start": chunk_start_ms, "end": prev_word_end_ms}
                )
                # 将当前词作为下一个新子句的开头
        ...
        if current_chunk:
            final_segments.append({"text": build_text(current_chunk), "start": chunk_start_ms, "end": prev_word_end_ms})
...
def faster_whisper(
    *,
    ...
    # ROCm 环境下 faster-whisper(ctranslate2) 不支持 GPU，自动回退 CPU
    if is_cuda and torch.version.hip is not None:
        logger.warning(
            "ROCm 环境检测: faster-whisper 依赖 ctranslate2 不支持 ROCm GPU，自动回退 CPU。如需 GPU 加速请使用 openai-whisper 识别渠道"
        )
        is_cuda = False
        compute_type = "int8"
    ...
    try:
        ...
        try:
            ...
            if (
                "model.bin is incomplete" in error
                or "json.exception.parse_error" in error
                or "EOF while parsing a value" in error
            ):
                msg = (
                    f"模型下载不完整，请删除目录 {local_dir}，重新下载"
                    if defaulelang == "zh"
                    else f"The model download may be incomplete, please delete the directory {local_dir} and download it again"
                )
        ...
        if speech_timestamps:
            ...
            clip_timestamps_dicts = [{"start": it[0] / 1000.0, "end": it[1] / 1000.0} for it in speech_timestamps]
...
def pipe_asr(
    ...
    try:
        ...
        results_iterator = p(inputs_generator(), generate_kwargs=generate_kwargs)
...
def qwen3asr_fun0(logs_file=None, is_cuda=False, audio_file=None, model_name="1.7B", device_index=0):  # gpu索引
    import torch, json
    ...
    try:
        ...
        model = Qwen3ASRModel.from_pretrained(
            ...
            forced_aligner_kwargs=dict(dtype=dtype, device_map=device_map, attn_implementation=atten),
        )
...
def qwen3asr_fun(
    cut_audio_list=None, logs_file=None, is_cuda=False, audio_file=None, model_name="1.7B", device_index=0  # gpu索引
):
    from pathlib import Path
    import torch, json
